/// Test de MinLSTM usando la implementación de la librería (xlstm::MinLstm)
...
fn test_gradients() {
    ...
    let config = MinLstmConfig { input_features: d, expansion_factor: expansion };
...
fn test_sequential_equivalence() {
    ...
    // Parallel forward
    let (out_parallel, _) = model.forward(x.clone(), Some(vec![MinLstmState::new(h0.clone())]));

    // Sequential forward (step by step)
    // Usamos g(h_0) para inicializar, al igual que hicimos con minGRU
    let mut h_prev = g(h0);
...
fn test_copy_task() {
    ...
    let config = MinLstmConfig { input_features: d, expansion_factor: expansion };
    let minlstm = config.init::<AdBackend>(&device);
    ...
    println!("\n--- TEST 3: LM Copy Task (S={seq_len}, B={batch_size}) [LIBRARY MinLstm] ---");
    ...
    for step in 1..=200 {
        ...
        for chunk_offset in (0..(row_len - seq_len)).step_by(seq_len) {
            ...
            let (out, _) = model.minlstm.forward(x_emb, Some(vec![MinLstmState::new(h0)]));
...
fn test_h0_persistence() {
    ...
    let (out_zeros, states_zeros) = model.forward(x.clone(), Some(vec![MinLstmState::new(h0_zeros)]));
    
    // Forward con h0 = estado anterior (simula persistencia)
    let h0_prev = states_zeros[0].hidden.clone();
    let (out_with_state, _) = model.forward(x.clone(), Some(vec![MinLstmState::new(h0_prev)]));
...
fn main() {
    println!("============================================");
    println!("  TEST SUITE: MinLSTM Library Implementation");
    println!("============================================\n");
