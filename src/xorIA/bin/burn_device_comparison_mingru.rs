...
fn run_copy_task<B: AutodiffBackend>(
    ...
    lr: f64,
    backend_name: &str,
    initial_record: Option<<MinGru<B> as Module<B>>::Record>,
) -> (f32, Vec<f32>) {
    let mut layer: MinGru<B> = if let Some(record) = initial_record {
    ...
    for i in 1..=steps {
        let (output, _) = layer.forward(input.clone(), None);
    ...
    let (final_output, _) = layer.forward(input, None);
    let final_output = final_output.into_data().as_slice::<f32>().unwrap().to_vec();
    (final_loss, final_output)
}

fn main() {
    println!("=== MINGRU DEVICE COMPARISON: CPU (NdArray) vs GPU (WGPU) ===");
    ...
    let config = MinGruConfig::new(embedding_dim).with_expansion_factor(2);

    // 1. Inicializar en CPU para extraer los pesos iniciales exactos
    let cpu_initial_model: MinGru<CpuBackend> = config.init(&cpu_device);
