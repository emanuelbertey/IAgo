...
use xlstm::{MinLstm, MinLstmConfig, MinLstmState};
...
impl<B: Backend> LanguageModelLayer<B> {
    pub fn forward(&self, x: Tensor<B, 3>, state: Option<Vec<MinLstmState<B>>>) -> (Tensor<B, 3>, Vec<MinLstmState<B>>) {
        let x = self.conv.forward(x.clone()) + x;
        
        let x_norm1 = self.norm1.forward(x.clone());
        let (output, next_state) = self.minlstm.forward(x_norm1, state);
        let x = x + output;
    ...
    pub fn step(&self, x_t: Tensor<B, 3>, conv_state: Tensor<B, 3>, minlstm_state: Tensor<B, 3>) -> (Tensor<B, 3>, Tensor<B, 3>, Tensor<B, 3>) {
        ...
        let x_norm1 = self.norm1.forward(x.clone());
        let (y_minlstm, next_minlstm_state) = self.minlstm.sequential_mode(x_norm1, minlstm_state);
        let x = x + y_minlstm;
        ...
        (x, next_conv_state, next_minlstm_state)
    }
}

#[derive(Module, Debug)]
pub struct MinLstmChatModel<B: Backend> {
...
impl<B: Backend> MinLstmChatModel<B> {
    pub fn forward(&self, input: Tensor<B, 2, Int>, states: Option<Vec<Vec<MinLstmState<B>>>>) -> (Tensor<B, 3>, Vec<Vec<MinLstmState<B>>>) {
    ...
    pub fn step(&self, input: Tensor<B, 1, Int>, conv_states: &mut Vec<Tensor<B, 3>>, minlstm_states: &mut Vec<Tensor<B, 3>>) -> Tensor<B, 2> {
        let [b] = input.dims();
        let mut x = self.embedding.forward(input.reshape([b, 1]));
        
        for i in 0..self.num_layers {
            let (out, next_conv, next_minlstm) = self.layers[i].step(x, conv_states[i].clone(), minlstm_states[i].clone());
            x = out;
            conv_states[i] = next_conv;
            minlstm_states[i] = next_minlstm;
        }
...
where <B as Backend>::FloatElem: num_traits::ToPrimitive {
    ...
    let mut minlstm_states = Vec::new();
    let b = 1;

    for i in 0..model.num_layers {
        conv_states.push(model.layers[i].conv.empty_state(b, device));
        let ml_hidden_size = model.hidden_size * 2;
        minlstm_states.push(Tensor::<B, 3>::zeros([b, 1, ml_hidden_size], device));
    }

    // Prefill context
    for &id in &ids {
        let input = Tensor::<B, 1, Int>::from_ints(vec![id as i32].as_slice(), device);
        let _ = model.step(input, &mut conv_states, &mut minlstm_states);
    }
    ...
    for _ in 0..length {
        ...
        let logits = model.step(input, &mut conv_states, &mut minlstm_states);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for _ in 0..num_layers {
        layers.push(LanguageModelLayer {
            ...
            minlstm: MinLstmConfig { input_features: hidden_size, expansion_factor: 2 }.init(&device),
    ...
    let mut model: MinLstmChatModel<MyBackend> = MinLstmChatModel {
    ...
    let hidden_size_expanded = hidden_size * 2; 
    
    println!("Iniciando entrenamiento ESTABLE (3 capas, Estilo MinLSTM con h_0 persistente)...");
    let mut h_states: Option<Vec<Vec<MinLstmState<MyBackend>>>> = None;
    ...
    for epoch in 0..25 {
        ...
        for b in 0..num_batches {
            ...
            h_states = Some(new_states.into_iter().map(|layer_s| {
                layer_s.into_iter().map(|s| MinLstmState::new(s.hidden.detach())).collect()
            }).collect());
